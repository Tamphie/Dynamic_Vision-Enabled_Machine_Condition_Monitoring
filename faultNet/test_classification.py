...
def parse_args():
    ...
    parser.add_argument('--num_category', default=4, type=int, choices=[4,10],  help='training on ModelNet10/40')
...
def test(model, loader, num_class=4, vote_num=1):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        y_pred.extend(pred_choice.cpu().numpy().tolist())
    ...
    for i in range(len(y_true)):
        confusion_mat[int(y_true[i]), int(y_pred[i])] += 1
    ...
    for first_index in range(len(confusion)):
        for second_index in range(len(confusion[first_index])):
            if first_index == second_index:
                plt.text(second_index, first_index, confusion[first_index][second_index], color='white', ha='center',
                         va='center')
            else:
    ...
    plt.show()
    plt.savefig("D:\\user\\xutongmiao\\confusion_mat.png", dpi=300)
